...
class SessionsView(ttk.Frame):
    ...
    def _build_ui(self):
        ...
        form = ttk.LabelFrame(self, text='Log Play Session', padding=14)
        ...
        ttk.Label(form, text='Sessions', style='Header.TLabel').grid(row=0, column=0, sticky='w', pady=(0, 12))
        ttk.Label(form, text='Game').grid(row=1, column=0, sticky='w')
        self.game_combo = ttk.Combobox(form, textvariable=self.game_var, state='readonly', width=28)
        self.game_combo.grid(row=2, column=0, sticky='ew', pady=(0, 8))
        ttk.Label(form, text='Date (YYYY-MM-DD)').grid(row=3, column=0, sticky='w')
        ...
        ttk.Entry(form, textvariable=self.duration_var, width=30).grid(row=6, column=0, sticky='ew', pady=(0, 8))
        ttk.Label(form, text='Notes').grid(row=7, column=0, sticky='w')
        self.notes_text = tk.Text(form, width=28, height=8, wrap='word', relief='flat')
        ...
        ttk.Label(table, text='Session History', style='Header.TLabel').grid(row=0, column=0, sticky='w')
        filter_bar = ttk.Frame(table)
        filter_bar.grid(row=0, column=0, sticky='e')
        self.filter_combo = ttk.Combobox(filter_bar, textvariable=self.filter_var, state='readonly', width=22)
    ...
    def refresh_games(self):
        ...
        if values and (not self.game_var.get() or self.game_var.get() not in self.game_map):
            self.game_var.set(values[0])
        if not self.filter_var.get() or (self.filter_var.get() != 'All games' and self.filter_var.get() not in self.game_map):
